...
协作工程只交付可在当前仓库和本地环境完成的实现与验证，包括源代码、测试、fixture、mock、本地构建和本地模拟器。不得规划、申请、准备、执行或依赖公网 IP、外网环境、云真机、远程硬件、发版、部署或 GitHub Issue、PR、Actions、Release、push 等操作。本地 Git 的状态、diff、scope 和提交审计不属于 GitHub 操作，仍按本规范执行。

任务涉及上述外部资源或操作时，必须缩小为可本地复现的路径，必要时使用 fixture、mock 或本地模拟器，并如实记录未执行外部验证。不得为了获取这些资源创建 blocker、权限申请或董事会门禁，也不得用本地证据声称已完成云真机、远程、发版或生产验证。业务规则、范围和方向等真实决策仍按董事会审批规则处理。
...
默认直接执行，不默认设门禁。调用已有本地 API、生成本地产物、修改 allowed 范围内文件、运行本地命令和重试本地工作，若影响有限、可恢复，均由 agent 直接完成并验证。排除的外部操作不得通过门禁重新引入；task 中出现“审批”“授权”或 “gate” 字样本身不构成门禁理由。
...
| `agent_actions` | 获批后的本地接口、文件、实现或验证动作；权限卡同时记录既有访问控制流程的配置或撤销动作；不得包含本地协作边界排除的操作、Paperclip 服务修改或生命周期控制 |
...
4. 董事会批准后由 agent 完成本地接口、文件和项目改动并验证；拒绝后取消依赖动作。
